...
(function () {
  ...
  function initCanvasRefs() {
    ...
    // Always reset — same DOM node every time, so must force-clear flags
    listenersBound     = false;
    canvas._a3mEvBound = false;
    return true;
  }

  // Exposed so index.html psLaunchDesigner can force a full re-bind
  ...
  // Base $5 · +$0.50 per element · size bonus
  const BASE_PRICE = 5.00;
  const PER_ELEMENT = 0.50;
  ...
  function updateDesignPrice() {
    ...
    layers.forEach(l => {
      ...
      const sizeBonus = Math.max(0, (area - 12000) / 12000) * 0.25;
      total += Math.min(sizeBonus, 2.0);
    });
    ...
    el.style.color = total > BASE_PRICE + 5 ? "#7c3aed" : total > BASE_PRICE ? "#f59e0b" : "#c9a84c";
  }
  ...
  function openDesigner(mode) {
    ...
    // Open modal FIRST — canvas needs layout paint before getContext works
    const dm = document.getElementById("designerModal");
    if (dm) dm.classList.add("open");

    // Reset binding flags so events re-register cleanly
    if (canvas && canvas._a3mEvBound) { canvas._a3mEvBound = false; }
    listenersBound = false;

    // Set title if mode given
    ...
    // Use rAF + small timeout: guarantees modal is painted before canvas ops
    requestAnimationFrame(() => {
      setTimeout(() => {
        if (!initCanvasRefs()) {          // always resets _a3mEvBound & listenersBound
          console.error("A3M: canvas not found after modal open");
          return;
        }
        clearCanvas();
        drawAll();
        ...
        addSmartPalette();
        translateDesignerUI();
        bindCanvasEvents();               // safe — initCanvasRefs reset the flags
  ...
  function bindCanvasEvents() {
    ...
    // Background color listener — bound once
    const bgEl = document.getElementById("bgColor");
    ...
    // Canvas pointer events — avoid double-binding
  ...
  document.addEventListener("DOMContentLoaded", function() {
    // Read canvas product type if selector exists
    var cp = document.getElementById("canvasProduct");
    if (cp) canvasProdType = cp.value;
    // DO NOT call initCanvasRefs/bindCanvasEvents here —
    // the canvas is inside a hidden modal and has zero dimensions.
    // openDesigner() handles all canvas init after the modal is visible.
...
// Make this file a valid ES module (imported by main.js)
export {};
