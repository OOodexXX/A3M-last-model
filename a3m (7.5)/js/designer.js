...
(function () {
  ...
  function summarizeDesign() {
    const texts = layers.filter(l => l.type === "text").map(l => l.text).slice(0, 5);
    const shapes = layers.filter(l => l.type !== "text").length;
    return [
      `منتج: ${canvasProdType}`,
      `عدد الطبقات: ${layers.length}`,
      `النصوص: ${texts.length ? texts.join(" / ") : "لا يوجد"}`,
      `الأشكال/الصور: ${shapes}`,
    ].join("\n");
  }
  ...
  function updateCanvasModeBadge() {
    ...
    if (title && !title.dataset.manual) {
      const map = { tshirt: "تيشرت", mug: "كوب", hat: "قبعة", bag: "حقيبة", paper: "ورقة" };
      title.textContent = `Design Studio • ${map[canvasProdType] || "منتج"}`;
    }
  }

  function addSmartPalette() {
    if (aiPanelInjected) return;
    const panel = document.querySelector(".d-panel");
    if (!panel) return;
    const section = document.createElement("div");
    section.className = "d-panel-section";
    section.id = "a3m-ai-panel";
    section.innerHTML = `
      <div class="d-panel-title">ذكاء التصميم</div>
      <div style="display:flex;flex-direction:column;gap:.5rem">
        <input class="d-input" id="a3mAiPrompt" placeholder="مثال: شعار أسود وذهبي لبراند ملابس"/>
        <div style="display:flex;gap:.4rem;flex-wrap:wrap">
          <button class="d-small-btn" type="button" id="a3mAiApplyBtn">✨ طبق</button>
          <button class="d-small-btn" type="button" id="a3mAiSuggestBtn">🧠 اقتراح</button>
          <button class="d-small-btn" type="button" id="a3mAiSummaryBtn">📋 ملخص</button>
        </div>
        <div style="font-size:12px;color:var(--txt3);line-height:1.7">
          اكتب وصفًا للتصميم، ثم اضغط <b>طبق</b>. كما يمكن للذكاء الاصطناعي في الشات أن يولّد تصميمًا ويطبّقه مباشرة.
        </div>
      </div>
    `;
    panel.insertBefore(section, panel.firstChild);
    ...
    section.querySelector("#a3mAiSuggestBtn").onclick = () => {
      const prompt = (document.getElementById("a3mAiPrompt") || {}).value || "";
      const spec = window.A3MDesignerAI ? window.A3MDesignerAI.suggestSpecFromPrompt(prompt) : null;
      if (spec) window.A3MDesignerAI.applyDesignSpec(spec, { source: "suggest" });
    };
    section.querySelector("#a3mAiSummaryBtn").onclick = () => {
      showToast(summarizeDesign());
    };
    aiPanelInjected = true;
  }
  ...
  function openDesigner(mode) {
    ...
    if (titleEl) {
      titleEl.dataset.manual = "1";
      titleEl.textContent = mode === "template" ? "Templates Mode" : "Scratch Mode";
    }
    clearCanvas();
    const dm = document.getElementById("designerModal");
    if (dm) dm.classList.add("open");
  ...
  function bindCanvasEvents() {
    if (!canvas || canvas._a3mBound) return;
    canvas._a3mBound = true;
    canvas.addEventListener("mousedown", _onPointerDown);
    canvas.addEventListener("mousemove", _onPointerMove);
    canvas.addEventListener("mouseup", _onPointerUp);
    canvas.addEventListener("mouseleave", _onPointerUp);
  ...
  function saveToCart() {
    const name = "Custom Design (" + canvasProdType + ")";
    if (typeof window.cart !== "undefined") {
      const ex = window.cart.find(i => i.id === "custom-design");
      if (ex) ex.qty += 1;
      else window.cart.push({ id: "custom-design", name, emoji: "🎨", price: 1500, qty: 1 });
    }
    if (typeof updateCart === "function") updateCart();
    if (typeof closeDesigner === "function") closeDesigner();
    showToast("Added ✦");
  ...
  function toggleVis(id) { const l = layers.find(x => x.id === id); if (l) { saveHist(); l.visible = !l.visible; drawAll(); } }
